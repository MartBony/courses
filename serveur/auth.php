	...
	if ($reqUser->rowCount() == 1) {
		...
		if (verifyPassword($pass, $user['pass'])) {
			if($user['activated']){
				...
				$updateClef->execute(array(hashPassword($clef), $user['id']));
				...
				if($_SERVER['SERVER_NAME'] != "localhost"){
						...
						<html><body>
							<section style="background: #eee;padding: 20px 10px;">
								...
								<section style="margin-top: 30px;width: 100%;">
									...
									<a style="color: white;font-size: 1.5em;font-weight:600;background: #0078d4;text-decoration: none;font-family: Segoe UI,Helvetica Neue,Helvetica,Arial,sans-serif;padding: 5px;" href="http://mprojects.fr/courses/serveur/activate.php?mail='.$mail.'&clef='.$clef.'">Cliquez ici</a> <br> <br>
									<hr> <br>
									<a style="color: black;font-size: 1.5em;font-family: Segoe UI,Helvetica Neue,Helvetica,Arial,sans-serif; font-weight: 700;color: #666;">Ou copiez/collez ce lien dans votre navigateur http://mprojects.fr/courses/serveur/activate.php?mail='.$mail.'&clef='.$clef.'</a><br><br><br><br>
					...
					if(mail($to, $subject, $message, $headers)){
						echo json_encode(array('status' => 400, 'payload' => array("type" => "WARNING", 'message' => 'Votre compte n\'est pas encore actif, nous vous envoyons un email pour confirmez votre authenticité.')));
					} else{
						echo json_encode(array('status' => 400, 'payload' => array('type' => "WARNING", 'message' => 'Votre compte n\'est pas encore actif mais l\'envoi d\'un email de confirmation à échoué.')));
					}

				} else echo json_encode(array('status' => 400, 'payload' => array('type' => "WARNING", 'message' => 'Le serveur local ne peut pas envoyer de mail de vérification.')));
...
function inscrire(PDO $bdd, $mail, $pass, $nom){
	...
	if ($reqUser->rowCount() == 0) {
		...
		while($resUniqueId != 0){
			...
			$resUniqueId = ($reqUniqueId->fetch())['nbr'];
		}

		$pass = hashPassword($pass);
		$clef = generateRandomString(30);

		$createUser = $bdd->prepare('INSERT INTO `users`
			(`mail`, `pass`, `userId`, `nom`, `clef`, `activated`, `hueColor`, `deleted`)
			VALUES (:mail,:pass,:userId,:nom,:clef, 0,:hueColor,0)');
		...
		$reqUser = $bdd->prepare('SELECT id FROM `users` WHERE mail = ? AND pass = ?');
		$reqUser->execute(array($mail, $pass));
		$user = $reqUser->fetch();

		$grpName = "Groupe de ${nom}";
		$insertGroupe = $bdd->prepare('INSERT INTO `groupes` (nom) VALUES (?)'); // Create groupe
		$insertGroupe->execute(array($grpName));
		
		$reqGroupe = $bdd->prepare('SELECT id FROM `groupes` WHERE nom = ? ORDER BY id DESC');
		$reqGroupe->execute(array($grpName));
		...
		if($_SERVER['SERVER_NAME'] != "localhost"){
			...
			// Sending email
			if(mail($to, $subject, $message, $headers)){
				echo json_encode(array('status' => 400, 'payload' => array("type" => "WARNING", 'message' => 'Vous êtes inscrits avec succès, nous vous demandons de confirmez votre adresse email avec le message que nous vous avons envoyé.')));
		 	} else{
				echo json_encode(array('status' => 400, 'payload' => array('type' => "WARNING", 'message' => 'Vous êtes correctement inscrits mais nous ne parvenons pas à vous envoyer un email de confirmation.')));
			}
